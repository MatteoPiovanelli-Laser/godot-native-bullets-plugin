...
class DynamicBulletsPool : public AbstractBulletsPool<DynamicBulletKit, DynamicBullet> {
	...
	bool _process_bullet(DynamicBullet* bullet, float delta) {
		...
		if(kit->speed_multiplier_over_lifetime.is_valid()) {
			...
			bullet->set_velocity(bullet->get_velocity().normalized() * bullet->starting_speed * speed_multiplier);
		}
		if(kit->rotation_offset_over_lifetime.is_valid()) {
			...
			float absolute_rotation = bullet->get_starting_transform().get_rotation() + rotation_offset;

			bullet->set_velocity(bullet->get_velocity().rotated(absolute_rotation - bullet->get_transform().get_rotation()));
		}
